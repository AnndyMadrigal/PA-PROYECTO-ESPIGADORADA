...
---Detalle
CREATE TABLE purchasing.purchase_order_details (
...
ALTER TABLE purchasing.purchase_order_details
ADD CONSTRAINT FK_POD_Order
FOREIGN KEY (order_id)
REFERENCES purchasing.purchase_orders(order_id);
GO
...
---SP
CREATE PROCEDURE purchasing.InsertPurchaseOrder
    @supplier_id INT,
    @created_by NVARCHAR(100),
    @details NVARCHAR(MAX) -- JSON con los productos
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        INSERT INTO purchasing.purchase_orders (supplier_id, created_by)
        VALUES (@supplier_id, @created_by);

        SET @order_id = SCOPE_IDENTITY();

        -- 2. Insertar detalle desde JSON
        INSERT INTO purchasing.purchase_order_details (order_id, product_id, quantity, unit_cost)
        SELECT 
            @order_id,
            product_id,
            quantity,
